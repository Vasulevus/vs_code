/****** Object:  StoredProcedure [16_enerho].[687815595_output_form]    Script Date: 12.12.2022 13:47:34 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
      ...
      IF @gen_table = 1 AND OBJECT_ID(concat('[db_depositarium].[16_enerho].[1296965689_output_form_',  @date_for, ']')) IS NULL 
		...
		SELECT @date_for = MAX([Dates]) FROM #Calenda WHERE LEFT([Dates],6) LIKE @date_for_cut;

		--	SELECT @date_for -- тест
            ...
            INTO #input_3_form
            FROM 
                  [db_archive].[16_enerho].[input_form3];
            ...
            WHERE       
				...
				AND
				LEFT(convert(nvarchar(MAX), cast(date_for as date) , 112),4) = LEFT(@date_for_cut,4)--Рік
            GROUP BY 
                  [company_name];
...
--			SELECT * FROM #P WHERE [I всього нак] IS NOT NULL AND [II всього нак] IS NOT NULL;
            ...
            SELECT 
                  *
            INTO 
                  [db_depositarium].[16_enerho].[687815595_output_form_' + @date_for + '] 
            FROM 
                  #Q 
            WHERE 
                  [F1493600066] IS NOT NULL
		AND 
                  [F1655390479] IS NOT NULL');
...
--			SELECT * FROM #P WHERE [I всього нак] IS NOT NULL AND [II всього нак] IS NOT NULL;
			...
			SELECT * FROM #Q WHERE [F1493600066] IS NOT NULL AND [F1655390479] IS NOT NULL;