USE [db_archive]
GO
/****** Object:  StoredProcedure [16_enerho].[16_energy_ta_add_json_to_input_forms_by_id]    Script Date: 26.10.2022 14:14:20 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
   ...
   --  JSON_QUERY( [payload],'$.sheets[0].data[8]') AS B
		JSON_VALUE([payload], '$.sheets[0].data[8]' )AS G
  FROM [Experiment + Work].[dbo].[in_report] WHERE title LIKE '777%'
...
SELECT 
     ...
     JSON_QUERY( [payload],'$.sheets[0].data[6]') AS B
	--	JSON_VALUE([payload], '$.sheets[0].data[0]' )AS G
  FROM [Experiment + Work].[dbo].[in_report] WHERE title LIKE '777%'
...
SELECT
JSON_VALUE(payload, '$.sheets[0].data[4][0]') AS F995390319
from  
	  [Experiment + Work].[dbo].[in_report] WHERE [title] = '777 Розширений макет для проведення дослідної експлуатації Відділ оперативного управління (Південне ТУОМ)за вересень 2022 р.'
