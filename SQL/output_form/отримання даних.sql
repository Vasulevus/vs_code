...
/*
set @date_for = (@date_for);
-- */

select [date_for] 
	   ,'[777].[254414212_output_form]' as [forma_777] 
       ,'F10' as [adress_777]
	   ,'' AS [sign]
	   --,'' as [umova]
	   ,'F20' as [adress_16]
...
from
	(select sum(cast([F_509940737] as int) ) as [val_16en], date_for, company_name
		  ...
		  where F_1517024731<>N'ВСЬОГО:' and company_name=N'Північне територіальне управління обслуговування мережі' group by date_for, company_name) r 
WHERE
[date_for] = @date_for
AND
val_16en IS NULL
