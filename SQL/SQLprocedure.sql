...
@min_date nvarchar(6), 
@max_date nvarchar(6), 
@grouping_level nvarchar(1)  -->> m - month, q - quartal, y - year
as 
begin
      declare @group_level nvarchar(max) --декларуємо змінні всередині batch
      declare @SQL nvarchar(max)
      --set @grouping_level='q', set @min_date='202003', set @max_date='202104'
      set @group_level = 
      case when @grouping_level = 'm' then 'SUBSTRING(r.[month_for],1,6)'
...
begin 
      ...
      ,sum(r.[F1326578739]*DATEDIFF(MINUTE,''0:00:00'' 
      ,r.[F_1689668618]))/SUM(DATEDIFF(MINUTE,''0:00:00'' 
      ,r.[F_1689668618])) as [F1326578739]
      ,concat(
      ...
      ,sum(r.[F_530524211]*DATEDIFF(MINUTE,''0:00:00'' ,r.[F1014490971]))/SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F1014490971])) as [F_530524211]
            ,
            concat(
            SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F1014490971]))/60,'' ����� '',
            SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F1014490971]))%60,'' ������''
            )
            as [F1014490971]
      ...
      ,sum(r.[F1933948865]*DATEDIFF(MINUTE,''0:00:00'' ,r.[F1880131690]))/sum(DATEDIFF(MINUTE,''0:00:00'' ,r.[F1880131690])) as [F1933948865]
            ,concat(
            SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F1880131690]))/60,'' ����� '',
            SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F1880131690]))%60,'' ������'')as [F1880131690]
      ...
      ,sum(r.[F436666917]*(DATEDIFF(MINUTE,''0:00:00'' ,r.[F_2109822435])))/SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F_2109822435])) as [F436666917]
            ,concat(
            SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F_2109822435]))/60, '' ����� '',
            SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F_2109822435]))%60, '' ������''
            ) as [F_2109822435]
      ...
      ,sum([F119626745]*(DATEDIFF(MINUTE,''0:00:00'' ,r.[F1322089514])))/SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F1322089514])) as [F119626745]
            ,concat(
            SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F1322089514])) /60, '' ����� '',
            SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F1322089514])) %60,'' ������''
            ) as [F1322089514]
      ...
      ,sum(r.[F_1568671225]*(DATEDIFF(MINUTE,''0:00:00'' ,r.[F_971945994])))/SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F_971945994])) as [F_1568671225]
            ,concat(
                  SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F_971945994])) /60,'' ����� '',
                  SUM(DATEDIFF(MINUTE,''0:00:00'' ,r.[F_971945994])) %60, '' ������''
                  ) as [F_971945994]
